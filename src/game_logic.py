...
# Goes over each row bool return and determines win for ALL rows:
...
# Check win in column one by one:
...
# Goes over each column bool return and determines win for ALL columns:
...
# Diagonoal win check top left-bottom right:
...
# Diagional win check top right-bottom left:
...
# Combines both diagonals checks to one:
...
# An altogether win check function that retrieves bool returns from all check functions:
